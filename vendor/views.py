...
def add_food(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            food.save()  # slug already set in model
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method =='POST':
            ...
            try:
                ...
                if hour:
                    ...
                    if day.is_closed:
                        response={'status':'success','id':hour.id,'day':day.get.display(),'is_closed':'Closed'}
                    else:
                        response={'status':'success','id':hour.id,'day':day.get.display(),'from_hour':hour.from_hour,'to_hour':to_hour}
...
def vendor_order_detail(request,order_number):
    try:
        ...
        ordered_food = OrderedFood.objects.filter(order=order, order__vendor__in=[vendor])
        context = {
            'order': order,
